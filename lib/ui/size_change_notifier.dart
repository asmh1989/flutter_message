...
class FXSizeChangedLayoutNotification extends LayoutChangedNotification{
    FXSizeChangedLayoutNotification(this.newSize):super();
    Size newSize;
}
...
///    widget tree.
class FXSizeChangedLayoutNotifier extends SingleChildRenderObjectWidget {
    /// Creates a [SizeChangedLayoutNotifier] that dispatches layout changed
    /// notifications when [child] changes layout size.
    const FXSizeChangedLayoutNotifier({
        Key key,
        Widget child
    }) : super(key: key, child: child);

    @override
    _FXRenderSizeChangedWithCallback createRenderObject(BuildContext context) {
        return new _FXRenderSizeChangedWithCallback(
            onLayoutChangedCallback: (size) {
                new FXSizeChangedLayoutNotification(size).dispatch(context);
            }
        );
    }
}

class _FXRenderSizeChangedWithCallback extends RenderProxyBox {
    _FXRenderSizeChangedWithCallback({
        RenderBox child,
