...
class CardEditState extends State<CardEdit>{
  ...
  Widget _getMenus({
    ...
    return new TextFormField(
      controller: controller ?? new TextEditingController(text: initString??''),
  ...
  Future<http.Response> _post([bool isGet = false, String no]) async {
    String url = Cache.instance.cdurl+(isGet?'/api/getnos.json': '/api/setnos.json');
      ...
      new SingleChildScrollView(
          child: new Form(
            ...
            child:  new Column(
                ...
                children: <Widget>[
                  _getMenus(preText: '设备卡号：', hintText: '请输入设备卡号', initString: _card.no,
                  ...
                  _getMenus(preText: '设备卡名：', hintText: '请输入设备卡名', initString:  _card.nnm,
                    onSave: (String value) => _card.nnm = value,
                    enable: _enable,
                  ),
                  _getMenus(preText: '操 作 员：', hintText: '请输入操作员', initString: _card.opnm,
                      onSave: (String value) => _card.opnm = value,
                      enable: _enable
                  ),
                  new InkWell(
                    onTap: !_enable? null : (){
                      DateTime time = new DateTime.fromMillisecondsSinceEpoch(_card.insdt * 1000);
                      Func.selectDate(context, time, (DateTime date){
                        setState(() {
                          _card.insdt = (date.millisecondsSinceEpoch ~/ 1000).toInt();
                        });
                      });
                    },
                    child: Padding(
                  ...
                  _getMenus(preText: '添加备注：', hintText: '请输入备注', initString:_card.re,
                    onSave: (String value) => _card.re = value,
                    enable: _enable
                  ),
                  _getMenus(preText: '安装地址：', hintText: '请输入安装地址', initString:_card.addr,
                  ...
                  new RaisedButton(
                    color: const Color(0xFF029de0),
                    highlightColor: const Color(0xFF029de0),
                    child: new Text(widget.card == null ? '确认添加':'确认修改', style: Style.loginTextStyle),
